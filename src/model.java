...
public class model {
    ...
    private static card getClass1(String player1) {
        if (player1.equals("rogue")) {
            return new rogue("Rogue", "Rogue", 2, 6, 6);
        }
        else if (player1.equals("archer")) {
            return new archer("Archer", "Archer", 8, 5, 4);
        }
        else if (player1.equals("shaman")) {
            return new shaman("Shaman", "Shaman", 4, 8, 3);
        }
        else if (player1.equals("barbarian")) {
            return new barbarian("Barbarian", "Barbarian", 10, 5, 2);
        }
        else {
            return new amalgam(player1, "Amalgamation", (int)Math.floor(Math.random() *(12 - 2 + 1) + 2), (int)Math.floor(Math.random() *(8 - 5 + 1) + 5), (int)Math.floor(Math.random() *(6 - 2 + 1) + 2));
        }
    }
    ...
    private static card getClass2(String player2) {
        if (player2.equals("rogue")) {
            ...
            return new amalgam(player2, "Amalgamation", (int)Math.floor(Math.random() *(12 - 2 + 1) + 2), (int)Math.floor(Math.random() *(8 - 5 + 1) + 5), (int)Math.floor(Math.random() *(6 - 2 + 1) + 2));
        }
    }
